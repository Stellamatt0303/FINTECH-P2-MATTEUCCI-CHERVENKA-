...
This is the Step 9 pass/fail pre-registration gate due Sunday, September 20, 2026. After the commit, copy its seven-character Git commit hash and quote that hash in every later project deliverable.
...
Do not change `strategy_rules.md` or `llm_analyst_prompt.md` after this pre-registration commit. Any later rule or prompt change invalidates the project and is subject to deduction code P0. The locked strategy code should also remain unchanged during the live evaluation phase.
